...
def plot_all_period_maps(ds, periods, suptitle = '', cmap = 'RdBu', col_increase = 1):
    '''
    ...
    nrows, ncols = (int(np.ceil(len(periods))) * 2, len(data_vars))
    
    # Extra columns
    ncols = ncols * col_increase
    
    fig = plt.figure(figsize = (8 * ncols, 5 * nrows))
    fig.suptitle('Percent of Years That are Stable', y=0.89, fontsize=15)
